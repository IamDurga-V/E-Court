...
.show {display: block;}
	...
	<title>Case Category|Supreme Court Of India</title>
...
window.onclick = function(event) {
  ...
  <div class="container">
    ...
    <a rel="Click Here"style="text-decoration:none;color:red;" href="casecategory.php"><h5>&#9679;Case Category</h5></a>
...
$result = mysqli_query($conn, 
     "CALL casec(@p0)") or die("Query fail: " . mysqli_error($conn));
if ($result->num_rows > 0) {
    // output data of each row
    
    while($row = $result->fetch_assoc()) {
        echo "<tr><td> " . $row["ccode"]."</td><td>".$row["category"]."</td><td>".$row["scategory"]."</td></tr>";
    }
} else {
    echo "0 results";
}
