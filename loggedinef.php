...
<ul>
  ...
  <li><a  href="anc.html">Notices &amp; Circulars</a></li>
    ...
    <li><a class="active" href="loggedinef.php">E-Filled</a></li>
...
$sql = "SELECT name,dob,address,state,district,pincode,gender,aadharno,phone,email,csubject,cfdetail FROM efile";
 $result = $conn->query($sql);
if ($result->num_rows > 0) {

    while($row = $result->fetch_assoc()) {
        echo "<tr><td> " . $row["name"]."</td><td> " . $row["dob"]."</td><td> " . $row["address"]."</td><td>".$row["pincode"]."</td><td>".$row["gender"]."</td><td>".$row["aadharno"]."</td><td>".$row["phone"]."</td><td>".$row["email"]."</td><td>".$row["csubject"]."</td><td>".$row["cfdetail"]."</td></tr>";
    }
} else {
    echo "0 results";
}
