...
<div class="card">
  ...
  <div class="container">
   <a rel="Click Here" style="text-decoration:none;" href="loggedinJudges.php"><h5>&#9679;Judges</h5></a>
    ...
    <a rel="Click Here" style="text-decoration:none;color:red;" href="loggedInCS.php"><h5>&#9679;Case Status</h5></a>
  ...
  <div class="centerbar">
      <h1><center>Case &amp; Judgement Details</center> </h1>
      <div class="tab">
...
<div id="Insert" class="tabcontent">
 <form action="cs_insert.php" method="POST" onsubmit="return executeOnSubmit();">
Case No.:<input type="text" name="cno" required >
Diary No.:<input type="text" name="dno" required >
Case Type:<input type="text" name="ctype" required>
Case Date:<input type="text" placeholder="YYYY-MM-DD" name="cyear" required>
Petitioner Name:<input type="text" name="pname" required >
Respondent Name:<input type="text" name="rname">
Respondent Advocates:<input type="text" name="rAd">
Petitioner Advocates:<input type="text" name="pAd">
Judges:<input type="text" name="judges" >
Next Hearing:<input type="text" placeholder="YYYY-MM-DD" name="nh"  >
...
$sql = "SELECT cno,diaryno,ctype,cyear,pname,rname,radvocates,padvocates,judges,nh,description FROM cs";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td> " . $row["cno"]."</td><td>".$row["diaryno"]."</td><td>".$row["ctype"]."</td><td>".$row["cyear"]."</td><td>".$row["pname"]."</td><td>".$row["rname"]."</td><td>".$row["radvocates"]."</td><td>".$row["padvocates"]."</td><td>".$row["judges"]."</td><td>".$row["nh"]."</td><td>".$row["description"]."</td><td>"." <button onclick=\"location.href='caseDetailsDelete.php?id=".$row['cno']."'\">Delete</button></td>"."</td></tr>";
    }
} else {
    echo "0 results";
}
